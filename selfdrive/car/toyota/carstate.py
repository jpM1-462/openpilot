...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam):
    ...
    if bool(cp.vl["PCM_CRUISE"]["CRUISE_ACTIVE"]) and not self.cruise_active_prev:
    ...
    if self.CP.hasZss:
      ...
      # default to stock if:
      # sensor going out of tolerance for more than 10 times
      # sensor offset has not been computed
      if self.out_of_tolerance_counter < 10 and not self.needs_angle_offset_zss:
        ...
        ret.steeringAngleDeg = self.stock_steer_value
        ret.usingZss = False
        # reset if user is in control
        if ret.steeringPressed and not self.steering_pressed_prev:
          self.needs_angle_offset_zss = True
          self.out_of_tolerance_counter = 0
        self.steering_pressed_prev = ret.steeringPressed
  ...
  def get_can_parser(CP):
    ...
    if CP.carFingerprint == CAR.PRIUS:
      ...
      checks.append(("AUTOPARK_STATUS", 0))
    if CP.hasZss:
