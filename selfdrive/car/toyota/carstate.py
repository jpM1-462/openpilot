...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.large_steer_previous = False
  ...
  def update(self, cp, cp_cam):
    ...
    if (self.stock_steer_value > 90 or self.stock_steer_value < -90) and not self.large_steer_previous and self.cruise_active:
      self.needs_angle_offset_zss = True # recompute offset when steer is over 90 degs
      self.out_of_tolerance_counter = 0 # Allow ZSS re-use after recomputation
    self.large_steer_previous = (self.stock_steer_value > 90 or self.stock_steer_value < -90)
