...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[], disable_radar=False):  # pylint: disable=dangerous-default-value
    ...
    if candidate == CAR.PRIUS and not ret.hasZss:
      ...
      tire_stiffness_factor = 0.6371   # hand-tune
      ret.mass = 3045. * CV.LB_TO_KG + STD_CARGO_KG
      set_lat_tune(ret.lateralTuning, LatTunes.TORQUE, MAX_LAT_ACCEL=1.7, FRICTION=0.06) # hand-tune
      ret.steerActuatorDelay = 0.3
    ...
    ret.longitudinalActuatorDelayLowerBound = 1.35
    ret.longitudinalActuatorDelayUpperBound = 1.45
  ...
  def _update(self, c):
    ...
    if self.CP.hasZss and not ret.usingZss and ret.cruiseState.enabled and not ret.vEgo == 0:
