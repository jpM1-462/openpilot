...
class CarController():
  ...
  def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd, hud_alert,
    ...
    pcm_decel_offset = interp(CS.out.vEgo, [0.0, 3.5], [0.18, 0.])
    ...
    if CS.CP.enableGasInterceptor and CS.CP.carFingerprint == CAR.PRIUS:
      ...
      if self.use_interceptor and enabled:
        ...
        # pcm_decel_offset to reduce ABS pump usage when OP is engaged
        MAX_INTERCEPTOR_GAS = interp(CS.out.vEgo, [0.0, 3.0, 3.5], [0.3, 0.5, 0.0])
        PEDAL_SCALE = interp(CS.out.vEgo, [0.0, 3.0], [2.5, 3.0])
        interceptor_gas_cmd = clip(actuators.accel / PEDAL_SCALE, 0., MAX_INTERCEPTOR_GAS)
        pcm_accel_cmd = pcm_decel_offset - max(0, -actuators.accel)
